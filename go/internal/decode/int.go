...
func (d *intDecoder) DecodeArray(arr arrow.Array, i int, p unsafe.Pointer) error {
	if arr.IsNull(i) {
		return nil
	}
	switch d.kind {
	case reflect.Int8:
		d.op(p, int64(arr.(*array.Int8).Value(i)))
	case reflect.Int16:
		d.op(p, int64(arr.(*array.Int16).Value(i)))
	case reflect.Int32:
		d.op(p, int64(arr.(*array.Int32).Value(i)))
	default:
		d.op(p, arr.(*array.Int64).Value(i))
	}
	return nil
}
